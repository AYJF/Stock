...
class SectorTable extends StatelessWidget {
  SectorTable(this.sourceData);

  final List<SectorsModel> sourceData;

  void _sort<T>(
    Comparable<T> Function(SectorsModel inv) getField,
    int colIndex,
    bool asc,
    _DataSource _src,
          ...
          columns: [
            DataColumn(
              ...
              onSort: (columnIndex, ascending) {
                _sort<String>((inv) => inv.sector, columnIndex, ascending,
                    _DataSource(context: context, rows: sourceData), _provider);
              },
            ),
            DataColumn(
              label: Text('changesPercentage'),
              onSort: (columnIndex, ascending) {
                _sort<String>(
...
class _DataSource extends DataTableSource {
  ...
  DataRow getRow(int index) {
    ...
    if (!row.changesPercentage.contains('-')) {
      _color = Colors.green;
      _icon = Icon(
  ...
  void sort<T>(Comparable<T> Function(SectorsModel d) getField, bool asc) {
    rows.sort((a, b) {
